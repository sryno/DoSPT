...
implicit none
  ...
  real*4, allocatable :: velocities(:,:,:), positions(:,:,:), volumes(:,:), positions1d(:)
  real*8, allocatable :: m(:), volumes_temp(:), volumes1d(:)
...
! Allocate arrays for trajectory input
  ...
  allocate( positions1d(1:natoms*3*n) )
  allocate( m(1:natoms) )
...
! Only calculate Voronoi cell volumes every 100 time steps
  ...
  allocate( volumes1d(1:natoms*n_volumes) )
!******************************************************
end subroutine
!=================================================================================================
!=================================================================================================
...
subroutine read_trajectory(n, natoms, tau, L, mode, positions, velocities, estimate_vel, error, &
  ...
  integer :: n, natoms, i, j, t1, t2, t3, t4, t5, t0, iostatus, di_volumes, idx
...
!     Begin modified READ block that vectorizes coordinates and volume information
...
! Convert positions from 3-index to 1-index
  idx=1
  do i=1,natoms
    do j=1,3
      do k=1,n
        positions1d(idx) = positions(i,j,k)
        idx = idx+1
      end do
    end do
  end do
! Convert volumes from 2-index to 1-index
  idx=1
  do i=1,natoms
    do j=1,n_volumes
      volumes1d(idx) = volumes(i,j)
      idx = idx+1
    end do
  end do
! Calculate voronoi volumes
  call voronoi_volumes(natoms, n, di_volumes, L, positions1d, volumes1d)
! Convert volumes from 1-index to 2-index
  idx=1
  do i=1,natoms
    do j=1,n_volumes
      volumes(i,j) = volumes1d(idx)
      idx = idx+1
    end do
  end do

! End modified READ block
